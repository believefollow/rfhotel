...
impl General {
    ...
    async fn id(&self, ctx: &Context<'_>) -> &str {
        ctx.data_unchecked::<Managers>().chars[self.0].id
    }

    /// The name of the human.
    async fn name(&self, ctx: &Context<'_>) -> &str {
        ctx.data_unchecked::<Managers>().chars[self.0].name
    }

    /// The friends of the human, or an empty list if they have none.
    async fn friends(&self, ctx: &Context<'_>) -> Vec<Character> {
        ctx.data_unchecked::<Managers>().chars[self.0]
            .friends
            .iter()
    ...
    async fn appears_in<'a>(&self, ctx: &'a Context<'_>) -> &'a [Gender] {
        &ctx.data_unchecked::<Managers>().chars[self.0].appears_in
    }

    /// The home planet of the human, or null if unknown.
    async fn home_planet<'a>(&self, ctx: &'a Context<'_>) -> &'a Option<&'a str> {
        &ctx.data_unchecked::<Managers>().chars[self.0].home_planet
    }
}
...
impl QueryRoot {
    async fn hero(
        ...
        gender: Gender,
            ...
            General(ctx.data_unchecked::<Managers>().luke).into()
    }
    ...
    async fn human(
        ...
        ctx.data_unchecked::<Managers>().human(&id).map(General)
    }
